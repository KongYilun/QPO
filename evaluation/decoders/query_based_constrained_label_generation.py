...
class QueryBasedConstrainedLabelGeneration(QueryBasedDecoder):
    """
    ...
    def decode(
        ...
        prompts = [self.template.render(instruction, demonstrations, test_example) for instruction, test_example in zip(instructions,test_examples)]
        ...
        #print('lc_prefix_ids:',lc_prefix_ids)
    ...
    def _get_verbalizer_perplexities(
        ...
        with torch.no_grad():
            input_ids = torch.tensor([completions[0]], dtype=int).to(model.device)
            tmp_output = model.hf_model.forward(input_ids=input_ids, past_key_values=past_key_values)
        if past_last_logit is None:
            # dummy past_last_logit if it is not passed. (just to get indexing to line up properly)
            past_last_logit = torch.zeros((1, tmp_output["logits"].shape[2]), dtype=float).to(model.device)
